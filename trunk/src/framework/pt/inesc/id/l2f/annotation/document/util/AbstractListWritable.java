...
public abstract class AbstractListWritable implements Writable, Configurable {
	 ...
	 */
	@SuppressWarnings("unchecked")
	private synchronized void addToList(Class clazz, byte id) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	protected synchronized void addToList(Class clazz) {
	...
	public void readFields(DataInput in) throws IOException {
		...
		for (int i = 0; i < newClasses; i++) {
			...
			try {
				this.addToList(Class.forName(className), id);
